...
Menu::Menu(size_t width, size_t height)
{
	...
	m_buttonsPointers[0] = new Button(sf::Vector2f(m_window->getSize().x / 2 - 150, m_window->getSize().y / 2.3f - 50), sf::Vector2i(300, 75.f), "resources/pressButton.png", "resources/unpressButton.png", "START");
	m_buttonsPointers[1] = new Button(sf::Vector2f(m_window->getSize().x / 2 - 150, m_window->getSize().y / 1.7f - 50), sf::Vector2i(300, 75.f), "resources/pressButton.png", "resources/unpressButton.png", "OPTIONS");
	m_buttonsPointers[2] = new Button(sf::Vector2f(m_window->getSize().x / 2 - 150, m_window->getSize().y / 1.7f + 100 - 50), sf::Vector2i(300, 75.f), "resources/pressButton.png", "resources/unpressButton.png", "CREDITS");
	m_buttonsPointers[3] = new Button(sf::Vector2f(m_window->getSize().x / 2 - 150, m_window->getSize().y / 1.7f + 200 - 50), sf::Vector2i(300, 75.f), "resources/pressButton.png", "resources/unpressButton.png", "EXIT");
	...
	m_backgroundTexture.loadFromFile("resources/menuBackground.png");
	...
	m_gameLogoTexture.loadFromFile("resources/menuLogo.png");
	...
	m_font.loadFromFile("resources/micross.ttf");
	m_gameVersion.setFont(m_font);
	m_gameVersion.setFillColor(sf::Color(32, 32, 32, 128));
	m_gameVersion.setString("Remastered V1.0");
	...
	if (!m_music.openFromFile("resources/menu.wav"))
	{
		std::cerr << "[!] Cannot load resource: 'resources/menu.wav'" << std::endl;
		std::cin.get();
		std::exit(1);
	}
	...
	if (!m_creditsTexture.loadFromFile("resources/credits.png"))
	{
		std::cerr << "[!] Cannot load resource: 'resources/credits.png'" << std::endl;
		std::cin.get();
		std::exit(1);
	}
	m_creditsSprite.setTexture(m_creditsTexture);
	//* Belum ada credits, segera ditambahkan
	
	m_options = false;

	m_optionReturnButton = new Button(sf::Vector2f(25, m_windowHeight - 100), sf::Vector2i(300, 75), "resources/pressButton.png", "resources/unpressButton.png", "RETURN TO MENU");
}
