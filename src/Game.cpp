...
void Game::Initialize(float musicVolume, float soundVolume, int numberOfLives){
	...
	{
		/* level file path */"resources/sample_level.txt",
		/* GUI font path */"resources/Cat.ttf",
		/* terrain sprite sheet path */"resources/sample_terraintextures.png",
		/* bomb sprite sheet path */"resources/sample_bombtextures.png",
		/* bomb ray sprite sheet path */"resources/sample_raytextures.png",
		/* player sprite sheet path */"resources/playersheets.png",
	};
	...
	if (!m_music.openFromFile("resources/game.wav"))
	{
		std::cerr << "[!] Cannot load resource: 'resources/game.wav'" << std::endl;
		std::cin.get();
		std::exit(1);
	}
...
void Game::update(float deltaTime){
	...
	{
		initGameplay(std::string("resources/sample_level.txt"));
	}
}
