...
Player::Player()
	...
	m_canBeDamaged(true)
{
	if (!m_soundBufferHit.loadFromFile("resources/hurt.wav"))
	{
		std::cerr << "[!] Cannot load resource: 'resources/hurt.wav'" << std::endl;
		std::cin.get();
		std::exit(1);
	}

	if (!m_soundBufferPlant.loadFromFile("resources/plant.wav"))
	{
		std::cerr << "[!] Cannot load resource: 'resources/plant.wav'" << std::endl;
		std::cin.get();
		std::exit(1);
	}
...
void Player::setFillColor(int i)
{
	if (i)
	{
